...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-16 lg:w-64 bg-[#0e1015] border-r border-white/5 flex flex-col shrink-0 shadow-2xl overflow-hidden transition-all duration-300">
      {/* ── Top: Logo + Nav (fixed height, never scrolls) ── */}
      <div className="p-3 lg:p-6 flex flex-col gap-5 shrink-0">
        {/* Logo */}
        ...
        <nav className="flex flex-col gap-1">
          ...
          {hasYtCookie && (
            ...
            >
              ...
              <span className="hidden lg:inline">YouTube Playlists</span>
            </button>
          )}
        </nav>
      </div>
      ...
      <div className={`flex-1 overflow-y-auto px-3 lg:px-6 py-4 flex flex-col gap-5 min-h-0 ${hasPlayer ? "pb-36" : "pb-4"}`}>

        {/* Local Playlists */}
        <div className="flex flex-col gap-1">
          <div className="flex items-center justify-center lg:justify-between px-1 mb-1">
          ...
          {playlists.length === 0 ? (
            <p className="text-xs text-slate-600 italic px-2 py-1 hidden lg:block">
              No playlists yet.
            </p>
          ) : (
            playlists.map((pl) => (
              <button
                ...
                className={`group flex items-center justify-center lg:justify-between px-3 lg:px-3 py-2 rounded-xl text-left transition-all duration-200 text-sm cursor-pointer ${
                  activeTab === pl.id
                    ? "bg-white/10 text-white font-medium border border-white/5"
                    : "text-slate-400 hover:text-slate-200 hover:bg-white/5"
                }`}
              >
                <div className="flex items-center gap-3 overflow-hidden justify-center lg:justify-start min-w-0">
                ...
                <button
                  ...
                  className="hidden lg:block opacity-0 group-hover:opacity-100 hover:text-red-300 p-1 rounded-md hover:bg-white/5 transition-all cursor-pointer shrink-0"
                >
