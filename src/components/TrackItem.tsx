...
import { Play, Pause, Loader2, Plus, Heart, Trash2, Clock } from "lucide-react";
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 min-w-0">
        ...
        <div className="w-12 h-12 rounded-xl overflow-hidden relative shrink-0 shadow-lg border border-white/5">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-slate-400 truncate mt-0.5">{track.uploaderName}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <div className="relative">
          <button
            onClick={(e) => {
              e.stopPropagation();
              setActiveDropdownTrackId(activeDropdownTrackId === track.videoId ? null : track.videoId);
            }}
          ...
          {activeDropdownTrackId === track.videoId && (
            <div className="absolute right-0 top-full mt-1 w-48 bg-[#161920] border border-white/10 rounded-xl shadow-2xl py-1 z-20" onClick={(e) => e.stopPropagation()}>
              <span className="block px-4 py-1.5 text-[9px] font-bold text-slate-500 tracking-wider uppercase">Add to Playlist</span>
              {playlists.map(p => (
                <button
                  key={p.id}
                  onClick={() => {
        ...
        >
          <Heart className={`w-4.5 h-4.5 ${isFavorite(track) ? "fill-pink-500" : ""}`} />
        </button>
